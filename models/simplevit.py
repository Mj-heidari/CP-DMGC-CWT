...
class PositionEmbedding3D(nn.Module):
    def __init__(self, in_channels, temporature: float = 10000.0):
        ...
        self.in_channels = in_channels
        inv_freq = 1.0 / (
            temporature ** (torch.arange(0, in_channels, 2).float() / in_channels)
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        emb = torch.zeros((a, b, c, self.in_channels * 3), device=x.device).type(
            x.type()
        )
        ...
        for _ in range(depth):
            self.layers.append(
                nn.ModuleList(
                    [
                        Attention(
                            hid_channels, heads=heads, head_channels=head_channels
                        ),
...
class SimpleViT(nn.Module):
    ...
    def __init__(
        ...
        self.to_patch_embedding = nn.Sequential(
            Rearrange(
                "b (c p0) (h p1) (w p2) -> b c h w (p1 p2 p0)",
                p0=t_patch_size,
                p1=patch_height,
                p2=patch_width,
            ),
        ...
        self.transformer = Transformer(
            hid_channels, depth, heads, head_channels, mlp_channels
        )

        self.linear_head = nn.Sequential(
            nn.LayerNorm(hid_channels), nn.Linear(hid_channels, num_classes)
        )
