...
class LaBraM(nn.Module):
    ...
    def __init__(self, chunk_size=1600, patch_size=200, out_chans=8, num_classes=1000, embed_dim=200, depth=12,
                 ...
                 use_mean_pooling=True, init_scale=0.001, use_abs_pos_emb=True, electrodes=None, **kwargs):
        super().__init__()
    ...
    def forward(self, x, electrodes=[], return_patch_tokens=False, return_all_tokens=False, **kwargs):
        # Accept dataloader output shape (B, N, T) and reshape internally
        # to (B, N, num_patches, patch_size) expected by the model.
        if x.dim() == 3:
            B, N, T = x.shape
            if T % self.patch_size != 0:
                raise ValueError(f"Input temporal length ({T}) is not divisible by patch_size ({self.patch_size})")
            # num_patches = T // self.patch_size
            x = rearrange(x, 'b n (p t) -> b n p t', t=self.patch_size)
