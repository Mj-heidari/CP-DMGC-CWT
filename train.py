...
def run_nested_cv(
    ...
    """
    Perform nested cross-validation and save RAW predictions for later analysis.
    No post-processing (MA, calibration) is done here.
    """
    ...
    for fold, (train_val_dataset, test_dataset) in enumerate(make_cv_splitter(dataset, **outer_cv_params)):
        ...
        for inner_fold, (train_dataset, val_dataset) in enumerate(inner_splits):
            ...
            # Training loop - only log validation metrics
            training_log = []
...
if __name__ == "__main__":
    ...
    # Run nested CV (only training and prediction)
    ...
    logging.info(f"Training completed. Raw predictions saved in {run_dir}")
    logging.info(f"Run 'python analyze_results2.py --run_dir {run_dir}' to analyze results")